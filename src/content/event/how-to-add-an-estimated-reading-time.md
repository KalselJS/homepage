...
const getPostsWithRT = async (posts: CollectionEntry<"event">[]) => {
...
Step (6) Refactor `getStaticPaths` of `/src/pages/events/[slug].astro` as the following
...
const getSortedPosts = async (posts: CollectionEntry<"event">[]) => {
